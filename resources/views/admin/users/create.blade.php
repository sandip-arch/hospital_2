        ...
        <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-6">
            ...
            <div x-show="selectedRole === 'driver'" class="p-5 rounded-2xl bg-amber-50/50 border border-amber-200 space-y-4" x-cloak>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <div>
                        <label class="block text-[11px] font-bold text-slate-700 mb-1">Assign Ambulance</label>
                        <select name="ambulance_id" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-amber-500">
                            <option value="">-- Unassigned / Later --</option>
                            @foreach($availableAmbulances as $amb)
                            <option value="{{ $amb->id }}">{{ $amb->vehicle_number }} ({{ $amb->model }})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
